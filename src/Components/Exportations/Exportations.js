...
export default function Exportation(){
    ...
    const columns = [
        ...
        {field: "Actions",headerName: "", flex: 2 ,headerAlign : 'center',align:'center',renderCell : (params)=>(
            <Button variant="text" startIcon={<Style />} sx={{ mt: 3, mb: 2 }} onClick={() => history.push({pathname: "/app/feedback/"+params.row.id , appState: { receivers : params.row.receiver } })}>{t('correspondence_follow')}</Button>
        )},        
    ]
    return(
        <React.Fragment>
